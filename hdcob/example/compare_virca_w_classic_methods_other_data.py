...
if __name__ == '__main__':
    ...
    data = pd.read_csv("demoPLS-EN.csv")
    header = list(data.columns)

    quantities_names = header[1:17]
    judges = header[17:]
    # judges = header[20]
    # judges = header[25]
    judges = header[20:22]
    melted_data = pd.melt(data, value_vars=judges, id_vars=header[0:17], var_name="Judge", value_name="Score")

    # One hot-encoding of the categorical variables
    juices = pd.get_dummies(melted_data["Orange juice"]).copy()
    judges_dumm = pd.get_dummies(melted_data["Judge"]).copy()

    x_obs = melted_data[quantities_names[:10]].copy()  # Observations [available components]
    x_obs = tensor(x_obs.to_numpy())

    x_miss = melted_data[quantities_names[10:]].copy()  # Missing observations and observed
    x_miss = tensor(x_miss.to_numpy())

    # cond = pd.concat([juices, judges_dumm], axis=1)  # Conditioning data
    # cond = pd.concat([juices], axis=1)  # Conditioning data
    cond = pd.DataFrame(data=[])
    cond = tensor(cond.to_numpy())

    target = melted_data["Score"].copy()  # Predictor judge scores
    target = tensor(target.to_numpy()[:, np.newaxis])

    # -----------------
    x_obs = data[quantities_names[10:]].copy()  # Observations [available components]
    x_obs = tensor(x_obs.to_numpy())

    x_miss = data[quantities_names[:10]].copy()  # Observations [available components]
    x_miss = tensor(x_miss.to_numpy())

    target = data[judges].copy()  # Predictor judge scores
    target = tensor(target.to_numpy())

    cond = (torch.zeros_like(x_miss)[:, 0]).unsqueeze(1)
    # cond = pd.get_dummies(data["Orange juice"]).copy()
    # cond = tensor(cond.to_numpy())
    ...
    # Save folder
    results_folder = os.path.join(os.getcwd(), "results_juice")
    os.makedirs(results_folder, exist_ok=True)

    # Multiple input features to predict one output
    NUM_SAMPLES = melted_data.shape[0]
    ...
    split = int(np.floor(NUM_SAMPLES * 0.5))
    ...
    list_scores = list()
    max_neigh = min(len(train_idx), 50)
    for n in range(1, max_neigh):
        scores = cross_val_score(KNeighborsRegressor(n_neighbors=n), x_input_knn, y_knn,
                                 scoring='neg_mean_squared_error', cv=max_neigh)
